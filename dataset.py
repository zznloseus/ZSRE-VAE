...
class Dataset(Dataset):
    ...
    def __init__(self, mode, data_file, description_file, description_file_processed, m, pretrained_model_name_or_path, max_len, model, args, use_mlm = False, expand_or_not = True):
        ...
        #roberta_large
        # self.head_mark_ids = 468 # #对应的token id，在prlm的vocab.txt里面定义
        # self.tail_mark_ids = 1374 # @同上
        self.model = model
        self.args = args
    ...
    def split_labels(self):
        ...
        if self.dataset == 'fewrel':
            fewrel_file_path = f'./data/rel2id/fewrel_rel2id/fewrel_rel2id_{self.m}_42.json'
            with open(fewrel_file_path, 'r', encoding='utf-8') as r2id:
            ...
            wiki_file_path = f'./data/rel2id/wikizsl_rel2id/wikizsl_rel2id_{self.m}_42.json'
            with open(wiki_file_path, 'r', encoding='utf-8') as r2id:
                relation2idx = json.load(r2id)
                train_relation2idx,dev_relation2idx, test_relation2idx = relation2idx['train'], relation2idx['eval'], relation2idx['test']
    ...
    def get_evaluate_des_features(self):
        ...
        print(rids)
        # logger.info("rids: {}".format(rids))
        # print(self.des_features)
