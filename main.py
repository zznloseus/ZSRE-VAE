...
def train(train_dataset, model, args, device):
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        print('Saveing Model...')
        args.checkpoint_dir = f'0519_checkpoints/one_stage_wo_OT_{args.dataset}_split_{args.seed}_unseen_{str(args.unseen)}_epoch_{epoch}.pth'
...
def evaluate(dataset, model, args, device):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs = model(**inputs)
            max_sim_idx, max_recon_sim_idx, sen_vec, des_vec, recon_vec = outputs
            # max_sim_idx, sen_vec, des_vec = outputs
            predict_labels_sim.extend(max_sim_idx.tolist())
            predict_labels_recon_sim.extend(max_recon_sim_idx.tolist())
        ...
        p_macro_recon_sim, r_macro_recon_sim, f_macro_recon_sim = compute_macro_PRF(np.array(predict_labels_recon_sim), np.array(true_labels))
        return p_macro_sim, r_macro_sim, f_macro_sim, p_macro_recon_sim, r_macro_recon_sim, f_macro_recon_sim
...
if __name__=='__main__':
    ...
    parser.add_argument("--input_pretrained_model_name_or_path", type=str, default='bert-base-uncased', help='huggingface pretrained model')
    parser.add_argument("--des_pretrained_model_name_or_path", type=str, default='bert-base-uncased', help='huggingface pretrained model')
    parser.add_argument("--add_auto_match", type=str, default='True', help='')
    args = parser.parse_args()

    
    if not os.path.exists('checkpoints'):
        os.makedirs('0514_checkpoints')
    ...
    model = EMMA(args.des_pretrained_model_name_or_path, add_auto_match, args.max_seq_len, args.k)
    ...
    train_dataset = Dataset("train", args.data_file, args.relation_description_file, 
                            ...
                            args.des_pretrained_model_name_or_path, args.max_seq_len, model, args, expand_or_not=args.expand_data)
    model, best_step, min_train_loss, total_steps= train(train_dataset, model, args, args.device)
    
    # model = torch.load("0519_checkpoints/one_stage_wo_OT_wikizsl_split_7_unseen_5_epoch_4.pth", weights_only=False)
    # dev
    train_dataset.mode = "dev"
    dev_dataset = train_dataset
    p_macro_sim, r_macro_sim, f_macro_sim, p_macro_recon_sim, r_macro_recon_sim, f_macro_recon_sim = evaluate(dev_dataset, model, args, args.device)
    # p_macro_sim, r_macro_sim, f_macro_sim = evaluate(dev_dataset, model, args, args.device)
    dev_info_sim = f'[dev][sim] (macro) final precision: {p_macro_sim:.4f}, recall: {r_macro_sim:.4f}, f1 score: {f_macro_sim:.4f}'
    print(dev_info_sim)
    dev_info_classify = f'[dev][recon_sim] (macro) final precision: {p_macro_recon_sim:.4f}, recall: {r_macro_recon_sim:.4f}, f1 score: {f_macro_recon_sim:.4f}'
    print(dev_info_classify)

    # test
    dev_dataset.mode = "test"
    test_dataset = dev_dataset
    p_macro_sim, r_macro_sim, f_macro_sim, p_macro_recon_sim, r_macro_recon_sim, f_macro_recon_sim = evaluate(test_dataset, model, args, args.device)
    # p_macro_sim, r_macro_sim, f_macro_sim = evaluate(test_dataset, model, args, args.device)
    test_info_sim = f'[test][sim] (macro) final precision: {p_macro_sim:.4f}, recall: {r_macro_sim:.4f}, f1 score: {f_macro_sim:.4f}'
    print(test_info_sim)
    test_info_classify = f'[dev][recon_sim] (macro) final precision: {p_macro_recon_sim:.4f}, recall: {r_macro_recon_sim:.4f}, f1 score: {f_macro_recon_sim:.4f}'
    print(test_info_classify)
    # running time
    end_time = time.time()
    run_time = end_time - start_time
    with open("result_model_recall_ir_ot_baseline_fewrel_64_0515.txt", "a") as file:
